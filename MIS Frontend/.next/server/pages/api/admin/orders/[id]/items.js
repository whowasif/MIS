...
async function handler(req, res) {
    ...
    try {
        ...
        const [items] = await db.execute(`SELECT oi.*, p.name AS joined_product_name,
       ...
       ORDER BY oi.id ASC`, [
        ...
        return res.status(200).json({
            items: JSON.parse(JSON.stringify(mapped))
        });
    } catch (error) {
...
var __webpack_exports__ = __webpack_require__.X(0, [9563,322], () => (__webpack_exec__(9195)));
