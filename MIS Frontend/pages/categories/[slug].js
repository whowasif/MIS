...
export const getServerSideProps = async ({ params }) => {
  ...
  try {
    ...
    const [specRows] = await db.execute(
      `SELECT spec_name, spec_label, display_order, category_id FROM category_specs
       WHERE category_id IN (${relatedPlaceholders}) AND is_filterable = 1
       ORDER BY (category_id = ?) DESC, display_order ASC`,
      [...relatedIds, category.id]
    )
    const specsByName = {}
    specRows.forEach((r) => {
      if (!specsByName[r.spec_name]) {
        specsByName[r.spec_name] = { spec_name: r.spec_name, spec_label: r.spec_label, display_order: r.display_order }
      }
    })
    ...
    try {
      const [optRows] = await db.execute(
        `SELECT spec_name, option_value FROM spec_filter_options
         WHERE category_id IN (${relatedPlaceholders})
         ORDER BY spec_name ASC, display_order ASC, option_value ASC`,
        relatedIds
      )
      optRows.forEach((r) => {
