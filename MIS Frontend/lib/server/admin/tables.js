...
import { managedTableNames } from '../../admin/managed-tables'

export const MANAGED_TABLES = managedTableNames

const AUTO_COLUMNS = new Set(['created_at', 'updated_at', 'deleted_at'])
...
export const getManagedTablesSummary = async () => {
  const db = getDbPool()
  const summary = []
...
const normalizeValueForWrite = (rawValue, column) => {
  ...
  if (['decimal', 'float', 'double'].includes(type)) {
    if (value === '') return column.isNullable ? null : 0
    const asNumber = Number(value)
    if (!Number.isFinite(asNumber)) throw new Error(`Invalid decimal for ${column.columnName}`)
    return asNumber
  }
...
const mapEditableColumns = (columns, includePrimary = false) =>
...
export const getTableRows = async ({ table, limit = 50, offset = 0 }) => {
  const safeTable = ensureManagedTable(table)
  const db = getDbPool()
  ...
  const [rows] = await db.query(
    `SELECT * FROM ${escapeIdentifier(safeTable)} ORDER BY ${escapeIdentifier(primaryKeyColumn)} DESC LIMIT ${safeLimit} OFFSET ${safeOffset}`
  )

  return rows
}

export const createTableRow = async ({ table, values }) => {
  const safeTable = ensureManagedTable(table)
  const db = getDbPool()
  const columns = await getTableColumns(safeTable)
  ...
  const query = `INSERT INTO ${escapeIdentifier(safeTable)} (${selectedColumns
...
export const updateTableRow = async ({ table, id, values }) => {
  const safeTable = ensureManagedTable(table)
  const db = getDbPool()
  const columns = await getTableColumns(safeTable)
  const primaryKeyColumn = getPrimaryKeyColumn(columns)
  ...
  const query = `UPDATE ${escapeIdentifier(safeTable)} SET ${assignments.join(', ')} WHERE ${escapeIdentifier(
    primaryKeyColumn
  )} = ? LIMIT 1`

  const [result] = await db.execute(query, [...assignmentValues, id])
  return result.affectedRows
}

export const deleteTableRow = async ({ table, id }) => {
  const safeTable = ensureManagedTable(table)
  const db = getDbPool()
  const columns = await getTableColumns(safeTable)
  const primaryKeyColumn = getPrimaryKeyColumn(columns)
