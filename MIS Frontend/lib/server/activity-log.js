 ...
 */
export const logAdminActivity = async ({ adminId, adminEmail, action, resource, resourceId = null, details = null, ipAddress = null }) => {
  try {
    const db = getDbPool()
    await db.execute(
      `INSERT INTO admin_activity_logs (admin_id, admin_email, action, resource, resource_id, details, ip_address, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, NOW())`,
      [adminId || null, adminEmail || null, action, resource, resourceId || null, details || null, ipAddress || null]
    )
  } catch (e) {
 ...
 */
export const logVisitorEvent = async ({ eventType, page = null, searchQuery = null, productId = null, productName = null, ipAddress = null, userAgent = null, referrer = null, customerId = null }) => {
  try {
    const db = getDbPool()
    await db.execute(
      `INSERT INTO visitor_logs (event_type, page, search_query, product_id, product_name, ip_address, user_agent, referrer, customer_id, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
 ...
 */
export const getAdminActivityLogs = async (limit = 100, offset = 0) => {
  const db = getDbPool()
  const safeLimit = Math.max(1, Math.min(500, Number(limit) || 100))
  const safeOffset = Math.max(0, Number(offset) || 0)

  const [rows] = await db.query(
    `SELECT * FROM admin_activity_logs ORDER BY created_at DESC LIMIT ${safeLimit} OFFSET ${safeOffset}`
  )
  return rows
}
 ...
 */
export const getAdminActivityLogsFiltered = async ({
  limit = 50, offset = 0, adminEmail = null, action = null, resource = null, resourceId = null, dateFrom = null, dateTo = null,
} = {}) => {
  const db = getDbPool()
  ...
  const [rows] = await db.query(
    `SELECT * FROM admin_activity_logs${whereClause} ORDER BY created_at DESC LIMIT ${safeLimit} OFFSET ${safeOffset}`,
    params
  )
  return { rows, total }
}
 ...
 */
export const getAdminActivityFilterOptions = async () => {
  const db = getDbPool()
  const [admins] = await db.query(
    `SELECT DISTINCT admin_email FROM admin_activity_logs WHERE admin_email IS NOT NULL AND admin_email <> '' ORDER BY admin_email ASC`
  )
  const [actions] = await db.query(
    `SELECT DISTINCT action FROM admin_activity_logs WHERE action IS NOT NULL AND action <> '' ORDER BY action ASC`
  )
 ...
 * Get visitor logs
 */
export const getVisitorLogs = async (limit = 100, offset = 0, eventType = null) => {
  const db = getDbPool()
  const safeLimit = Math.max(1, Math.min(500, Number(limit) || 100))
  const safeOffset = Math.max(0, Number(offset) || 0)
...
export const getVisitorStats = async (days = 7) => {
  ...
  const [pageViews] = await db.query(
    `SELECT COUNT(*) as total FROM visitor_logs WHERE event_type = 'page_view' AND created_at >= DATE_SUB(NOW(), INTERVAL ? DAY)`,
    [days]
  )
  const [searches] = await db.query(
    `SELECT COUNT(*) as total FROM visitor_logs WHERE event_type = 'search' AND created_at >= DATE_SUB(NOW(), INTERVAL ? DAY)`,
    [days]
  )
  const [productViews] = await db.query(
    `SELECT COUNT(*) as total FROM visitor_logs WHERE event_type = 'product_view' AND created_at >= DATE_SUB(NOW(), INTERVAL ? DAY)`,
    [days]
  )
  const [uniqueVisitors] = await db.query(
    `SELECT COUNT(DISTINCT ip_address) as total FROM visitor_logs WHERE created_at >= DATE_SUB(NOW(), INTERVAL ? DAY)`,
    [days]
  )
  const [topPages] = await db.query(
    `SELECT page, COUNT(*) as visits FROM visitor_logs WHERE event_type = 'page_view' AND created_at >= DATE_SUB(NOW(), INTERVAL ? DAY) GROUP BY page ORDER BY visits DESC LIMIT 10`,
    [days]
  )
  const [topSearches] = await db.query(
    `SELECT search_query, COUNT(*) as count FROM visitor_logs WHERE event_type = 'search' AND created_at >= DATE_SUB(NOW(), INTERVAL ? DAY) AND search_query IS NOT NULL GROUP BY search_query ORDER BY count DESC LIMIT 10`,
    [days]
  )
  const [topProducts] = await db.query(
    `SELECT product_name, product_id, COUNT(*) as views FROM visitor_logs WHERE event_type = 'product_view' AND created_at >= DATE_SUB(NOW(), INTERVAL ? DAY) AND product_name IS NOT NULL GROUP BY product_id, product_name ORDER BY views DESC LIMIT 10`,
    [days]
  )
